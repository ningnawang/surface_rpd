...
MshLoader::MshLoader(const std::string& filename) {
  ...
  if (sizeof(int) != 4) {
    std::cerr << "Error: code must be compiled with int size 4 bytes."
              << std::endl;
    throw NOT_IMPLEMENTED;
  }
...
void MshLoader::parse_nodes(std::ifstream& fin) {
  ...
  if (m_binary) {
    ...
    for (size_t i = 0; i < num_nodes; i++) {
      int node_idx =
          *reinterpret_cast<int*>(&data[i * (4 + 3 * m_data_size)]) - 1;
      m_nodes[node_idx * 3] =
          *reinterpret_cast<Float*>(&data[i * (4 + 3 * m_data_size) + 4]);
      m_nodes[node_idx * 3 + 1] = *reinterpret_cast<Float*>(
          &data[i * (4 + 3 * m_data_size) + 4 + m_data_size]);
      m_nodes[node_idx * 3 + 2] = *reinterpret_cast<Float*>(
          &data[i * (4 + 3 * m_data_size) + 4 + 2 * m_data_size]);
    }
...
void MshLoader::parse_node_field(std::ifstream& fin) {
  ...
  for (size_t i = 0; i < num_real_tags; i++) fin >> real_tags[i];

  fin >> num_int_tags;
  int* int_tags = new int[num_int_tags];
  for (size_t i = 0; i < num_int_tags; i++) fin >> int_tags[i];
    ...
    for (size_t i = 0; i < num_entries; i++) {
      ...
      for (size_t j = 0; j < num_components; j++) {
        field[node_idx * num_components + j] =
            *reinterpret_cast<Float*>(&data[base_idx + j * m_data_size]);
      }
    }
    delete[] data;
  } else {
...
int MshLoader::num_nodes_per_elem_type(int elem_type) {
  ...
  switch (elem_type) {
    ...
    case 5:
      ...
      break;
    default:
      std::cerr << "Warning: Element type (" << elem_type
